...
int main(int, char**)
{
  ...
  //instantiation of CarSpeed
    CarSpeed car_speed(HTthreshold,Sobelthreshold,frame,MAX_MATCH_ERROR);
  ...
  for(size_t now=0;loop;now++)
  {
    ...
    float Speed =car_speed.GetStreetSpeed(frame,now,features); //get normalized z component of speed
